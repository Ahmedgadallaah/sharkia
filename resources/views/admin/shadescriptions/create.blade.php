...
<form action="{{url ('/admin/shadescription/store') }}" method="post" enctype="multipart/form-data">
{{ csrf_field() }}
 ...
 <div class="card-body" id="arabic-form">
    <div class="form-group">
        ...
        <select name="type" class="form-control select2 select2-hidden-accessible" style="width: 100%;height:auto;" data-select2-id="1" tabindex="-1"  aria-hidden="true">
            <option value="line" >{{ trans('message.lines') }}</option>
            <option value="map" >{{ trans('message.map') }}</option>
            <option value="feast" >{{ trans('message.feast') }}</option>
            <option value="about" >{{ trans('message.about') }}</option>
        </select>
      </div>
     </div>
