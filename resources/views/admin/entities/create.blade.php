...
<form action="{{url ('/admin/entity/store') }}" method="post" enctype="multipart/form-data">
{{ csrf_field() }}
...
<div class="form-group">
    ...
    <select name="type" class="form-control select2 select2-hidden-accessible" style="width: 100%;" data-select2-id="1" tabindex="-1"  aria-hidden="true">
        <option value="city" >{{ trans('message.city') }}</option>
        <option value="company" >{{ trans('message.company') }}</option>
        <option value="directorate" >{{ trans('message.directorate') }}</option>
